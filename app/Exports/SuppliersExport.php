...
class SuppliersExport implements FromCollection, WithHeadings, WithMapping, WithEvents, QueueShouldQueue
{
    protected $fields;
    ...
    {
        $suppliers = Supplier::select($this->fields)->get();

        foreach ($suppliers as $index => $supplier) {
            $supplier->row_number = $index + 1;
        }
    ...
    {
        $mappedData = [$supplier->row_number];

        foreach ($this->fields as $field) {
            if ($field === 'contact_info') {
                $info = json_decode($supplier->contact_info, true) ?? [];
                $parts = [];
                ...
                $mappedData[] = $supplier->is_active ? 'Active' : 'Non-Active';
            } else {
                $mappedData[] = $supplier->{$field} ?? '';
            }
        }
